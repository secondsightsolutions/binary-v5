...
func CryptInit(certStrPemB64, caCertStrPemB64, keyStrPemB64, keyStrPemEncrB64, saltEncrB64, passphrase string) (*tls.Certificate, *x509.Certificate, error) {
	...
	if len(caCertStrPemB64) == 0 {
		return nil, nil, fmt.Errorf("no CA certificate")
	}
	if len(keyStrPemB64) == 0 && len(keyStrPemEncrB64) == 0 {
		return nil, nil, fmt.Errorf("no encoded private key")
	}
	...
	if saltEncrB64 != "" && passphrase != "" {
		if saltEncr, err := hex.DecodeString(saltEncrB64); err == nil {
			if decr, err := Decrypt(saltEncr, passphrase); err == nil {
				...
				return nil, nil, fmt.Errorf("error decrypting salt: %s", err.Error())
			}
		} else {
			return nil, nil, fmt.Errorf("error decoding salt: %s", err.Error())
		}
	}
	...
	if certBpem, err = base64.StdEncoding.DecodeString(certStrPemB64); err != nil {
		return nil, nil, fmt.Errorf("cannot B64-decode server certificate: %s", err.Error())
	}
    if caCertBpem, err = base64.StdEncoding.DecodeString(caCertStrPemB64); err != nil {
		return nil, nil, fmt.Errorf("cannot B64-decode CA certificate: %s", err.Error())
	}
	...
	if keyStrPemB64 != "" {
		if keyBpem, err = base64.StdEncoding.DecodeString(keyStrPemB64); err != nil {
			if keyStrPemEncrB64 == "" {
				return nil, nil, fmt.Errorf("cannot B64-decode private key: %s", err.Error())
			}
		}
	}
	if len(keyBpem) == 0 {
		if keyStrPemEncrB64 != "" {
			if keyBpem, err = base64.StdEncoding.DecodeString(keyStrPemEncrB64); err != nil {
				...
				return nil, nil, fmt.Errorf("cannot B64-decode encrypted private key: %s", err.Error())
			} else if passphrase != "" {
				...
				if keyBpem, err = Decrypt(keyBpem, passphrase); err != nil {
					...
					return nil, nil, fmt.Errorf("cannot decrypt private key: %s", err.Error())
				}
				// If we get to here, we decrypted the PEM. Good.
			} else {
				return nil, nil, fmt.Errorf("no passphrase provided to decrypt private key")
			}
		}
	}
	if len(keyBpem) == 0 {
		return nil, nil, fmt.Errorf("could not create private key PEM") // Don't think we'd ever get here.
	}
	...
	if !X509pool.AppendCertsFromPEM(caCertBpem) {
		return nil, nil, fmt.Errorf("cannot append CA cert to cert pool")
	}

	if TLSCert, err = tls.X509KeyPair(certBpem, keyBpem); err != nil {
		return nil, nil, fmt.Errorf("cannot load TLS key-pair: %s", err.Error())
	}
	if block, _ := pem.Decode(certBpem); block != nil {
		if X509cert, err = x509.ParseCertificate(block.Bytes); err != nil {
			return nil, nil, fmt.Errorf("cannot parse x509 certificate from server certificate PEM: %s", err.Error())
		}
	} else {
		return nil, nil, fmt.Errorf("cannot PEM decode server certificate from server certificate PEM")
	}
...
func HasOU(xcrt *x509.Certificate, ou string) bool {
	if xcrt != nil {
		for _, certOU := range xcrt.Subject.OrganizationalUnit {
...
func X509cname(xcrt *x509.Certificate) string {
	if xcrt != nil {
		return xcrt.Subject.CommonName
	}
	return ""
}
func X509ou(xcrt *x509.Certificate) string {
	if xcrt != nil {
		return strings.Join(xcrt.Subject.OrganizationalUnit, ",")
	}
	return ""
}

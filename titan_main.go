...
func run_titan(wg *sync.WaitGroup, opts *Opts, stop chan any) {
    ...
    titan = &Titan{srv: &binaryV5SvcServer{}, pools: map[string]*pgxpool.Pool{}, opts: opts}

    titan.getEnv()
    titan.pools["citus"]  = db_pool(titan.cit_db_host, titan.cit_db_port, titan.cit_db_name, titan.cit_db_user, titan.cit_db_pass, true)
    titan.pools["binary"] = db_pool(titan.rbt_db_host, titan.rbt_db_port, titan.rbt_db_name, titan.rbt_db_user, titan.rbt_db_pass, true)
    ...
    svcWGrp.Add(2)
    go run_datab_ping(   svcWGrp, stop, 60, titan.pools)
    go run_grpc_server(  svcWGrp, stop, "service", svcp, RegisterBinaryV5SvcServer, titan.srv)
    go run_save_to_azure(svcWGrp, stop, 5, azac, azky)
    go run_save_to_datab(svcWGrp, stop, 5, azac, azky, titan.pools)
    svcWGrp.Wait()
}

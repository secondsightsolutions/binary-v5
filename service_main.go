...
func service_main(wg *sync.WaitGroup, stop chan any) {
	...
	go run_grpc_services(svcWGrp, stop, "service", "23461", RegisterBinaryV5SvcServer, service.srv)
...
func (svc *Service) getEnv() {
	...
	setIf(&svc.cit_db_name, "CIT_DB_PORT")
	setIf(&svc.cit_db_pass, "CIT_DB_NAME")
	setIf(&svc.cit_db_port, "CIT_DB_USER")
	setIf(&svc.cit_db_user, "CIT_DB_PASS")
	setIf(&svc.esp_db_host, "ESP_DB_HOST")
	setIf(&svc.esp_db_name, "ESP_DB_PORT")
	setIf(&svc.esp_db_pass, "ESP_DB_NAME")
	setIf(&svc.esp_db_port, "ESP_DB_USER")
	setIf(&svc.esp_db_user, "ESP_DB_PASS")
	setIf(&svc.rbt_db_host, "RBT_DB_HOST")
	setIf(&svc.rbt_db_name, "RBT_DB_PORT")
	setIf(&svc.rbt_db_pass, "RBT_DB_NAME")
	setIf(&svc.rbt_db_port, "RBT_DB_USER")
	setIf(&svc.rbt_db_user, "RBT_DB_PASS")
